"""
Seed ASSESS visa applications and milestones (Step 3 of 3).
Creates VisaApplication records and ApplicationMilestones for realistic case tracking.

Run order:
1. seed_assess_beneficiary_users.py
2. seed_assess_case_groups.py
3. seed_assess_visa_apps.py (this file)
...
from app.models.visa import VisaApplication, VisaTypeEnum, VisaStatus, VisaCaseStatus, VisaPriority, VisaType
...
def seed_assess_visa_apps():
    """Seed ASSESS visa applications and milestones."""
    db = SessionLocal()
    
    try:
        print("\n📋 Seeding ASSESS visa applications and milestones...")
        ...
        # Get visa types
        eb2niw_type = db.query(VisaType).filter(VisaType.code == "EB2NIW").first()
        h1b_type = db.query(VisaType).filter(VisaType.code == "H1B").first()
        tn_type = db.query(VisaType).filter(VisaType.code == "TN").first()
        
        if not eb2niw_type or not h1b_type or not tn_type:
